...
class LibroController{
    ...
    async update(req,res){
        try {
            const libro = req.body;
            const [result] = await pool.query('UPDATE Libros SET nombre=(?), autor=(?), categoria=(?), yearpublicacion=(?), ISBN=(?) WHERE ISBN=(?)',[libro.nombre, libro.autor, libro.categoria, libro.yearpublicacion, libro.ISBN]);
        if(result.affectedRows > 0) {
